...
This bridge maps a validated `BridgeJob` to a **static high-level flight-request plan**, and separately tracks a real link-loss heartbeat. `UavCoordinator` has no MAVLink or vendor SDK dependency, so it can be verified on Windows, Linux or CI without a real UAV. It emits only a named flight request - `ARM`, `TAKEOFF`, `GOTO_WAYPOINT`, `HOVER_AND_CAPTURE` or `RETURN_TO_LAUNCH` - never a real attitude/throttle command.

**Honesty fix (found in an ecosystem-wide software-improvements audit, UAV-02):** this request used to be named `PRE_FLIGHT_CHECK`. The real MAVLink command it sends (`MAV_CMD_COMPONENT_ARM_DISARM`, arm) genuinely arms the vehicle - PX4/ArduPilot run their own pre-arm check suite as part of processing an arm request, so there is no separate, real, standard MAVLink command that runs those checks without arming. Naming it "check" made a real arm command sound like an inert, read-only query. Renamed to `ARM`, and `mavlink_transport.py`'s own `send()` now requires an explicit `confirm_arm=True` before actually sending it - a real arm command needs its own deliberate opt-in, not just the same shared cell/machine gate every other request already passes through.
...
`mavlink_transport.py` is this bridge's first real transport: `MavlinkFlightControl.send()` maps each request to a real, numbered `MAV_CMD` from the authoritative common.xml spec and sends it as a real `COMMAND_LONG` - `ARM` -> `MAV_CMD_COMPONENT_ARM_DISARM` (400, arm, requires `confirm_arm=True`); `TAKEOFF` -> `MAV_CMD_NAV_TAKEOFF` (22); `GOTO_WAYPOINT` -> `MAV_CMD_DO_REPOSITION` (192, the real immediate-GUIDED-mode command, not the mission-item `MAV_CMD_NAV_WAYPOINT`); `HOVER_AND_CAPTURE` -> `MAV_CMD_NAV_LOITER_UNLIM` (17) then `MAV_CMD_IMAGE_START_CAPTURE` (2000); `RETURN_TO_LAUNCH` -> `MAV_CMD_NAV_RETURN_TO_LAUNCH` (20); `LAND` -> `MAV_CMD_NAV_LAND` (21). A rejected `UavDispatch` is never sent. `open_mavlink_connection()` is the one place `pymavlink` (optional `[mavlink]` extra) is imported, lazily.

**Found in the same audit (UAV-01):** `takeoff_altitude_m`/`waypoint_lat`/`waypoint_lon`/`waypoint_alt_m` are now validated before ever reaching a real `COMMAND_LONG` - non-finite (`NaN`/`+-inf`) values are always refused, latitude/longitude are checked against the real, universal +/-90/+/-180 geographic range, and altitude must be a finite, positive number. A maximum operating altitude is deliberately NOT enforced here - that is a real, jurisdiction/deployment-specific profile parameter (e.g. FAA Part 107's own ceiling for small UAS), not a universal physical constant, and remains real, deferred future work rather than an invented number.
