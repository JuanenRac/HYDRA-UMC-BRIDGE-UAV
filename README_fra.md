...
* ✅ **Vocabulaire réel de requêtes de vol nommées :** `ARM`, `TAKEOFF`, `GOTO_WAYPOINT`, `HOVER_AND_CAPTURE`, `RETURN_TO_LAUNCH` — jamais une commande brute d'assiette/accélérateur. Un `COMPLETE` de mission normale et un `ABORT` d'urgence se résolvent tous deux vers la même requête réelle `RETURN_TO_LAUNCH`. *(implémenté)*
